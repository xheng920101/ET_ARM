#include "include.h"
//extern uint8_t IDfromEEP[4];
//extern uint8_t JIGID[1];
uint8_t IDfromEEP[4];
uint8_t JIGID[1];
 ...
 */
 ErrorStatus OTPSequence(void)
 {
	...
	buf[0] = 0x03;	// enable access OxE1
	 ///----------------ASUS5.5 R63350  ID--------------------------///
	buf[3] = JIGID[0]; //DDB0 ID1[15:8]
	buf[4] = IDfromEEP[0]; //DDB1 ID1[7:0]
	buf[5] = IDfromEEP[1]; //DDB2 ID2[15:8]
	buf[6] = IDfromEEP[2]; //DDB3 ID2[7:0]
	buf[7] = IDfromEEP[3]; //DDB4 ID3[7:0]
	 ///----------------------------------------------///
	buf[8]++;
